...
x_reszt = zeros(Mlen, 1); 
T_reszt = 41;
k=1;
%%  d)  U¯YWANIE PRÓBEK Z PLIKU ZAMIAST SZUMU
%pob = audioread('coldvox.wav');
%%
...
for  nr = 1 : Nramek
    ...
    %% a) IGNOROWANIE DECYZJI O  U/UV
    %T = 0; 
    %% b) DWUKROTNE OBNI¯ENIE CZÊSTOTLIWOŒCI TONU PODSTAWOWEGO G£OSKI DWIÊCZNEJ
    %{    
        if T>0
            T = 2*T;
        end
    %}
    %% c) ZMIANA CZÊSTOTLIWOŒCI TONU PODSTAWOWEGO NA STA£¥
    %T = 80;
    %% 
    ...
    if nr == 19
        ...
        %%{
        field = ((nr-1)*Mstep+1):((nr-1)*Mstep+1+Mlen);
        x_reszt = filter([1;a], 1, x(field));
        T_reszt = T;
        figure(11);
        plot(x_reszt, 'b.-');
        hold all;
        ...
        title('G³oska dŸwiêczna "a"');
        xlabel('Nr próbki');
        legend('Sygna³ resztkowy', 'Sygna³ oryginalny');
        %}
    elseif nr == 20
        figure(5);
        hold all;
    ...
    for n=1:Mstep
        ...
        %ss(n)=wzm*pob(j)-bs*a;		% filtracja 'syntetycznego' pobudzenia d)
        %ss(n)=wzm*pob-bs*a;		% filtracja 'syntetycznego' pobudzenia
        %ss(n)=wzm*x_reszt(mod(j, T_reszt))-bs*a;		% filtracja jednym okresem sygnalu resztkowego
        if nr > 19
            ss(n)=wzm*x_reszt(mod(2, T_reszt))-bs*a;
            k = k+1;
        else
            ss(n)=wzm*pob-bs*a;
        end
    ...
    %disp(pobudzenie);   %% ODKOMENTOWAC DO d)
